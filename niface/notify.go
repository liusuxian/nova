...
type INotify interface {
	...
	NotifySaveOfflineMsg(ctx context.Context, id uint32, msgID uint16, f MsgDataFunc, offlineMsg IOfflineMsg, callback ...SendCallback) // 通知连接，当连接不存在或处于关闭状态时保存离线消息
}

// IOfflineMsg 离线消息接口
type IOfflineMsg interface {
	Save(ctx context.Context, id uint32, msgID uint16, f MsgDataFunc) (err error) // 保存离线消息
	Get(ctx context.Context, id uint32) (msgList []OfflineMsg, err error)         // 获取离线消息
}
