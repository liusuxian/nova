...
type IRequest interface {
	...
	SetCtxVal(key CtxKey, value any)              // 将键值对作为自定义参数设置到请求的 Context 中
	GetCtxVal(key CtxKey, def ...any) (value any) // 检索并返回给定键名的值，可选参数 def 指定如果请求的 Context 中不存在给定的 key 时的默认值
